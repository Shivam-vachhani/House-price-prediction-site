...
const PredictionForm = ({
  ...
  const handleSubmit = () => {
    setIsLoading(true);
    // Show spinner briefly, then fire the real onSubmit
    setTimeout(() => {
      setIsLoading(false);
      onSubmit();
    }, 600);
  ...
  return (
    <>
      <style>{`
        ...
        .pf-btn:not(:disabled):hover::after {
          background: rgba(255,255,255,0.06);
        }
        .pf-btn-loading {
          background: #5a6f9e !important;
          cursor: not-allowed;
        }
      `}</style>
      ...
      <div className="bg-gray-50 border border-gray-200 rounded-2xl p-4 sm:p-6 mb-6">
        ...
        <button
          ...
          disabled={isLoading}
          className={`pf-btn inline-flex items-center gap-2 bg-[#6E83B2] text-white text-sm font-semibold px-5 sm:px-6 py-2.5 rounded-lg w-full sm:w-auto justify-center sm:justify-start ${isLoading ? "pf-btn-loading" : "hover:bg-[#5a6f9e]"}`}
        >
          {isLoading ? (
            <>
